  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      if (delta) {
        ...
        fullReply += delta;
        tokenBatch++;
        tok += delta.length; // count chars for more accurate tok/s display
        // Batch DOM updates — render every N tokens OR every RENDER_INTERVAL_MS
        if (tokenBatch % 6 === 0) {
    ...
    // Final flush: ensure last partial batch is rendered
    flushRender();
